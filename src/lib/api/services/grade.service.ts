...
export class GradeService extends BaseService {
  async getStudentGrades(
    ...
    return this.get<GetGradPerTermYearByStudentIdResponse>(
      GRADE_ENDPOINTS.GET_STUDENT_GRADE_BY_TERM_YEAR,
      params
    );
  }

  async getStudentsBelowGrade(
    params: GetStudentIfGradeBelowRequest
  ): Promise<GetGradBelowResponse[]> {
    return this.get<GetGradBelowResponse[]>(
      GRADE_ENDPOINTS.GET_STUDENT_IF_GRADE_BELOW,
      params
    );
  }

  async getGroupGradesAbove(
    params: GetStudentGradeAboveRequest
  ): Promise<GetStudentGroupGradeAboveResponse[]> {
    return this.get<GetStudentGroupGradeAboveResponse[]>(
      GRADE_ENDPOINTS.GET_GROUP_GRADE_ABOVE,
      params
    );
  }

  async getStudentTranscript(
    params: GetStudentTranscriptRequest
  ...
  async getGradeSummary(
    ...
    return this.get<GetGroupSummaryGradeResponse>(
      GRADE_ENDPOINTS.GET_GROUP_SUMMARY_GRADE,
      params
    );
  }
  ...
  async GetStudentDetailAndSummaryScoreByStudentCode(
    ...
    return this.get<GetStudentDetailAndSummaryScoreByStudentCodeResponse>(
      `${GRADE_ENDPOINTS.GET_STUDENT_DETAIL_AND_SUMMARY_SCORE_BY_STUDENT_CODE}?studentCode=${studentCode}`
    );
  }
  ...
  async UpsertStudentGrades(
    ...
    return this.post<boolean>(GRADE_ENDPOINTS.UPSERT_STUDENT_GRADES, payload);
  }
